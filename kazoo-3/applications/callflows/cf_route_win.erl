...
execute_callflow(JObj, Call) ->
    case should_restrict_call(Call) of
        'true' ->
            lager:debug("endpoint is restricted from making this call, terminate exit", []),
        ...
        {'false', Call2} ->
            ...
            bootstrap_callflow_executer(JObj, Call2)
    end.
...
-spec execute_fraudcontrol_callflow(whapps_call:call()) -> whapps_call:call().
execute_fraudcontrol_callflow(Call) ->
    case should_frauded_call(Call) of
        'true_account' ->
            lager:debug("account is frauded from making this call, terminate exit", []),
            _ = whapps_call_command:answer(Call),
            _ = whapps_call_command:prompt(<<"cf-account_is_frauded_terminate">>, Call),
            _ = whapps_call_command:queued_hangup(Call);
        'true_reseller' ->
            lager:debug("reseller is frauded from making this call, terminate exit", []),
            _ = whapps_call_command:answer(Call),
            _ = whapps_call_command:prompt(<<"cf-account_is_frauded_reseller_terminate">>, Call),
            _ = whapps_call_command:queued_hangup(Call);
        'false' -> Call
    end.

-spec should_frauded_call(whapps_call:call()) -> 'true_account'|'true_reseller'|'false'.
should_frauded_call(Call) ->
    case whapps_config:get_is_true(?CF_CONFIG_CAT, <<"calls_required_fraudcheck">>, 'true') of
        'false' ->
            lager:debug("!!! In sysconfig/callflows is 'calls_required_fraudcheck' = false");
        'true' ->
            lager:debug("sysconfig/ecallmgr is 'calls_required_fraudcheck' = true"),
            AccountId = whapps_call:account_id(Call),
            AccountFraudAmount = get_fraudamount(AccountId),
            AccountFraudCounted = cf_util:fraud_calc(AccountId,<<"24">>),
            case AccountFraudAmount < AccountFraudCounted of
                'true' ->
                        lager:debug("limit reached of Account ~s fraudamount ~p < ~p",[AccountId, AccountFraudAmount, AccountFraudCounted]), 'true_account';
                'false' ->
                        lager:debug("limit not reached of Account ~s fraudamount ~p < ~p",[AccountId, AccountFraudAmount, AccountFraudCounted]),
                        ResellerId = wh_services:find_reseller_id(AccountId),
                        ResellerFraudAmount = get_fraudamount(ResellerId),
                        ResellerFraudCounted = cf_util:fraud_calc(ResellerId,<<"24">>),
                        case ResellerFraudAmount < ResellerFraudCounted of
                            'true' ->
                                    lager:debug("limit reached of ResellerAccount ~s fraudamount ~p < ~p",[ResellerId, ResellerFraudAmount, ResellerFraudCounted]), 'true_reseller';
                            'false' ->
                                    lager:debug("limit not reached of ResellerAccount ~s fraudamount ~p < ~p",[ResellerId, ResellerFraudAmount, ResellerFraudCounted]), 'false'
                        end
            end
    end.

-spec get_fraudamount(ne_binary) -> ne_binary().
get_fraudamount(AccountId) ->
    case wh_services:is_reseller(AccountId) of
        'true' -> DefaultFraudAmount = '20';
        'false' -> DefaultFraudAmount = '50'
    end,
    case kz_account:fetch(AccountId) of
        {'error', _E} ->
            lager:warning("could not open account doc ~s : ~p", [AccountId, _E]),
            whapps_config:get_integer(?CF_CONFIG_CAT, <<"default_fraudamount">>, DefaultFraudAmount);
        {'ok', Doc} ->
            case wh_json:get_float_value([<<"limits">>, <<"fraud">>, <<"perday">>], Doc) of
                'undefined' -> whapps_config:get_integer(?CF_CONFIG_CAT, <<"default_fraudamount">>, DefaultFraudAmount);
                 _Res -> _Res
            end
    end.
...
should_restrict_call('undefined', _Call) -> {'false', _Call};
should_restrict_call(EndpointId, Call) ->
...
maybe_classification_restriction(JObj, Call) ->
    ...
    lager:debug("classified number ~s as ~s, testing for call restrictions JObj:~p"
                ,[Number, Classification, JObj]
               ),
    Deny = wh_json:get_value([<<"call_restriction">>, Classification, <<"action">>], JObj),
    Call2 = whapps_call:set_caller_classifier(Classification, Call),
    case Deny =:= <<"deny">> of
        'false' -> {'false', Call2};
        'true' -> 'true'
    end.
...
bootstrap_callflow_executer(_JObj, Call) ->
    Routines = [fun store_owner_id/1
                ...
                ,fun execute_fraudcontrol_callflow/1
...
set_language(Call) ->
    ...
    case cf_endpoint:get(Call) of
        ...
        {'error', _E} ->
            lager:debug("no source endpoint language to owned account / sys_default (~s)", [Default]),
...
update_ccvs(Call) ->
    ...
    lager:info("bootstrapping with caller id type ~s: \"~s\" ~s ~p"
               ,[CallerIdType, CIDName, CIDNumber, Call]
              ),
    Props = props:filter_undefined(
              [{<<"Hold-Media">>, cf_attributes:moh_attributes(<<"media_id">>, Call)}
               ,{<<"Caller-ID-Name">>, CIDName}
               ,{<<"Caller-ID-Number">>, CIDNumber}
               ,{<<"Caller-Classifier">>, wh_json:get_value(<<"caller_classifier">>, Call)}
