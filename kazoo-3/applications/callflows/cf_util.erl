...
-spec fraud_calc(ne_binary(), integer()) -> list().
fraud_calc(AccountId, Hours) ->
    To = calendar:datetime_to_gregorian_seconds({date(), time()}),
    Hours1 = list_to_integer(binary_to_list(Hours)),
    From = To - (3600 * Hours1),
    IsReseller = wh_services:is_reseller(AccountId),
    case wh_transactions:fetch_local(AccountId, From, To) of
        {'error', _R}=Error -> lager:debug("Not possible to fraud_calc:~p",[Error]);
        {'ok', Transactions} ->
            Sumlists = [fraudsum_calc(Transaction, IsReseller)
                         || Transaction <- wh_transactions:filter_for_per_minute(Transactions)
                      ],
            lists:sum(Sumlists)
    end.

-spec fraudsum_calc(list(), ne_binary()) -> list().
fraudsum_calc(List, IsReseller) ->
    Json = wh_transaction:to_public_json(List),
    case IsReseller of
        'false' ->
            wh_json:get_value(<<"amount">>, Json);
        'true' ->
            wh_json:get_value(<<"subamount">>, Json)
    end.