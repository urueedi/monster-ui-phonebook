...
caller_id(Attribute, Call) ->
    lager:debug("debug of call variables:~p", [Call]),
    CCVs = whapps_call:custom_channel_vars(Call),
    Inception = whapps_call:inception(Call),
    AccountId = whapps_call:account_id(Call),
    Name = whapps_call:caller_id_name(Call),
    CidFormat = whapps_call:callerid_format(Call),
    CidPrefix = whapps_call:callerid_prefix(Call),
    E164 = wnm_util:to_e164(whapps_call:caller_id_number(Call), AccountId),
    Int = wnm_util:e164_tointernational(E164),
    case CidFormat of
        'e164'          -> CallerId = E164;
        'international' -> CallerId = Int;
        'national'      -> CallerId = wnm_util:e164_tonational(E164);
        'local'         -> CallerId = wnm_util:e164_tonational(E164);
        'nochange'      -> CallerId = whapps_call:caller_id_number(Call);
        []              -> CallerId = E164;
        'nomatch'       -> CallerId = E164;
        'undefined'     -> CallerId = E164
    end,
    lager:info("caller id convert to format Name:<~s> CLID:~s, Cid-Format:~s CidPrefix:~s", [Name, CallerId, CidFormat, CidPrefix]),
    case (Inception =/= 'undefined'
          andalso not wh_json:is_true(<<"Call-Forward">>, CCVs))
        orelse wh_json:is_true(<<"Retain-CID">>, CCVs)
    of
        'true' ->
            maybe_normalize_cid(CallerId, Name, 'false', Attribute, Call);
        'false' ->
...
maybe_prefix_cid_number(Number, Name, Validate, Attribute, Call) ->
    case whapps_call:kvs_fetch('prepend_cid_number', Call) of
        'undefined' -> maybe_suffix_cid_number(Number, Name, Validate, Attribute, Call);
        Prefix ->
            lager:debug("prepending caller id number with ~s", [Prefix]),
            Prefixed = <<(wh_util:to_binary(Prefix))/binary, Number/binary>>,
            maybe_suffix_cid_number(Prefixed, Name, Validate, Attribute, Call)
    end.

-spec maybe_suffix_cid_number(ne_binary(), ne_binary(), boolean(), ne_binary(), whapps_call:call()) ->
                                     {api_binary(), api_binary()}.
maybe_suffix_cid_number(Number, Name, Validate, Attribute, Call) ->
    case whapps_call:kvs_fetch('suffix_cid_number', Call) of
        'undefined' -> maybe_prefix_cid_name(Number, Name, Validate, Attribute, Call);
        Suffix ->
            lager:debug("suffixing caller id number with ~s", [Suffix]),
            Suffixed = <<Number/binary, (wh_util:to_binary(Suffix))/binary>>,
            lager:debug("caller id number with ~s", [Suffixed]),
            maybe_prefix_cid_name(Suffixed, Name, Validate, Attribute, Call)
    end.
