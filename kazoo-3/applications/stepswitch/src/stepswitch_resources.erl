...
-export([fetch_global_resources/0
         ,fetch_global_resources/1
         ,fetch_global_resources/3

         ,fetch_local_resources/1
         ,fetch_local_resources/2

         ,global_resources_lcrprior/3
        ]).
...
-define(DEFAULT_LCRROUTE, whapps_config:get_binary(?SS_CONFIG_CAT, <<"default_lcrroute">>, <<"false">>)).
-define(DEFAULT_LCRMARGIN, whapps_config:get_binary(?SS_CONFIG_CAT, <<"default_lcrmargin">>, <<"0">>)).
-define(DEFAULT_CALLER_ID_TYPE,whapps_config:get_binary(?SS_CONFIG_CAT, <<"default_caller_id_type">>, <<"external">>)).
-define(DEFAULT_PROGRESS_TIMEOUT,whapps_config:get_integer(?SS_CONFIG_CAT, <<"default_progress_timeout">>, 8)).
...
-record(gateway, {
           ...
           ,password :: api_binary()
           ,route :: api_binary()
           ,gate_prefix = <<>> :: ne_binary()
           ,gate_suffix = <<>> :: ne_binary()
           ,gate_format = <<>> :: ne_binary()
           ,codecs = [] :: ne_binaries()
...
endpoints(Number, OffnetJObj) ->
    case maybe_get_endpoints(Number, OffnetJObj) of
        [] -> [];
        Endpoints ->
            AccountId = wh_json:get_value(<<"Account-ID">>, OffnetJObj),
            case maybe_priotity_by_lcrroute(AccountId) of
                'undefined' ->
                    case whapps_config:get_is_true(?SS_CONFIG_CAT, <<"default_lcrroute">>) of
                        'true' -> Endpoints;
                        _R -> lager:debug("Sysconfig lcrroute = ~s", [_R]), sort_endpoints(Endpoints)
                    end;
                'false' -> lager:debug("Account lcrroute = false"), sort_endpoints(Endpoints);
                {'true', _Margin} -> Endpoints
            end
    end.

-spec maybe_priotity_by_lcrroute(ne_binary()) -> 'ok'.
maybe_priotity_by_lcrroute(AccountId) ->
    case kz_account:fetch(AccountId) of
        {'error', _E} -> lager:warning("could not open account doc ~s : ~p", [AccountId, _E])
                        ,'undefined';
        {'ok', Doc} ->
            case wh_json:get_value(<<"priotity_by_lcrroute">>, Doc, 'undefined') of
                'true' ->
                        Margin = wh_json:get_binary_value(<<"priotity_by_lcrmargin">>, Doc, <<"0">>)
                        ,lager:debug("account lcrmargin:~p", [Margin])
                        ,{'true', Margin};
                _R -> lager:debug("account lcrroute = ~s Doc:~p", [_R, Doc])
                        ,_R
            end
    end.
...
get_local_endpoints(AccountId, Number, OffnetJObj) ->
    ...
    Resources = filter_resources(Flags, get(Number, AccountId)),
...
get_global_endpoints(Number, OffnetJObj) ->
    ...
    Flags = wapi_offnet_resource:flags(OffnetJObj, []),
    AccountId = wapi_offnet_resource:account_id(OffnetJObj),
    Resources = filter_resources(Flags, get(Number, AccountId, 'global')),
    resources_to_endpoints(Resources, Number, OffnetJObj).
...
endpoint_ordering(P1, P2) ->
    lager:debug("endpoint sort:~p/~p", [P1, P2]),
    ...
    wh_json:from_list(
      props:filter_empty(
        [{<<"Route">>, gateway_dialstring(Gateway, Number)}
         ,{<<"Caller-Classifier">>, wapi_offnet_resource:caller_classifier(OffnetJObj)}
         ,{<<"Callee-Classifier">>, wapi_offnet_resource:callee_classifier(OffnetJObj)}
         ,{<<"Callee-ID-Name">>, wapi_offnet_resource:outbound_callee_id_name(OffnetJObj)}
...
get(Number) ->
    case wh_cache:fetch_local(?STEPSWITCH_CACHE, {'global_resources-', Number}) of
        {'ok', Resources} -> Resources;
        {'error', 'not_found'} -> fetch_global_resources(Number)
    end.

-spec get(api_binary(), api_binary()) -> resources().
get(_Number, AccountId) ->
    case wh_cache:fetch_local(?STEPSWITCH_CACHE, {'local_resources', AccountId}) of
        {'ok', Resources} -> Resources;
        {'error', 'not_found'} -> fetch_local_resources(AccountId)
    end.

-spec get(api_binary(), api_binary(), api_binary()) -> resources().
get(Number, AccountId, 'global') ->
    fetch_global_resources(Number, AccountId, 'global').
...
-spec fetch_global_resources(ne_binary()) -> resources().
fetch_global_resources(Number) ->
    lager:debug("global resource cache miss, fetching from db with number:~s", [Number]),
    case whapps_config:get_is_true(?SS_CONFIG_CAT, <<"default_lcrroute">>) of
        'false' ->
            ViewOptions = ['include_docs'],
            case couch_mgr:get_results(?RESOURCES_DB, ?LIST_RESOURCES_BY_ID, ViewOptions) of
                ...
                {'ok', JObjs} ->
                    ...
                    Docs = [wh_json:get_value(<<"doc">>, JObj) || JObj <- JObjs],
                    Resources = resources_from_jobjs(Docs),
                    wh_cache:store_local(?STEPSWITCH_CACHE, {'global_resources-', Number}, Resources, CacheProps),
                    Resources
            end;
        'true' ->
            %% By LCR-Routesort
            SortDocs = lcr_util:lcrrates_for_did(Number, 'outbound'),
            Resources = resources_from_jobjs(SortDocs),
            Resources
    end.

-spec fetch_global_resources(ne_binary(), ne_binary(), api_binary()) -> resources().
fetch_global_resources(Number, AccountId, 'global') ->
    lager:debug("global resource from db with number:~s AccountId:~s", [Number, AccountId]),
    case maybe_priotity_by_lcrroute(AccountId) of
        'false' ->
                global_resources_prior(Number, AccountId);
        {'true', Margin} ->
                global_resources_lcrprior(Number, AccountId, Margin);
        'undefined' ->
            case whapps_config:get_is_true(?SS_CONFIG_CAT, <<"default_lcrroute">>, 'false') of
                'true' ->
                    global_resources_lcrprior(Number, AccountId, whapps_config:get_binary(?SS_CONFIG_CAT, <<"default_lcrmargin">>, <<"0">>));
                'false' ->
                    global_resources_prior(Number, AccountId)
            end
    end.

-spec global_resources_prior(ne_binary(), ne_binary()) -> resources().
global_resources_prior(Number, _AccountId) ->
        ViewOptions = ['include_docs'],
        case couch_mgr:get_results(?RESOURCES_DB, ?LIST_RESOURCES_BY_ID, ViewOptions) of
            {'error', _R} ->
                lager:warning("unable to fetch global resources: ~p", [_R]),
...
-spec global_resources_lcrprior(ne_binary(), ne_binary(), integer()) -> resources().
global_resources_lcrprior(Number, AccountId, Margin) ->
        %% By LCR-Routesort not cached!
        case hon_util:rates_did(Number, <<"outbound">>, AccountId) of
            'false' -> Price = 9.99;
            {Price,Resellprice} -> lager:debug("get rates VP:~p EP:~p MG:~p",[Price, Resellprice, Margin])
        end,
        Margin1 = wh_util:to_float(Margin),
        Maxprice = wh_util:to_float(Price) * (100 - Margin1) / 100,
        lager:debug("~p ~p ~p MP:~p",[Number, <<"outbound">>, AccountId, Maxprice]),
        SortDocs = lcr_util:lcrrates_for_did(Number, <<"outbound">>, AccountId, wh_util:to_binary(Maxprice)),
        Resources = resources_from_jobjs(SortDocs),
        Resources.
...
resource_from_jobj(JObj) ->
    Resource = #resrc{id=wh_doc:id(JObj)
                      ...
                      ,gate_prefix=wh_json:get_ne_value(<<"gate_prefix">>, JObj, <<>>)
                      ,gate_suffix=wh_json:get_ne_value(<<"gate_suffix">>, JObj, <<>>)
                      ,gate_format=wh_json:get_ne_value(<<"gate_format">>, JObj, <<>>)
                     },
...
resource_weight(JObj) when not is_integer(JObj) ->
    ...
    lager:debug("resource_weight:~p", [JObj]),
...
gateway_from_jobj(JObj, #resrc{is_emergency=IsEmergency
    ...
    #gateway{endpoint_type=EndpointType
             ...
             ,gate_prefix=wh_json:get_ne_value(<<"gate_prefix">>, JObj, <<>>)
             ,gate_suffix=wh_json:get_ne_value(<<"gate_suffix">>, JObj, <<>>)
             ,gate_format=wh_json:get_ne_value(<<"gate_format">>, JObj, <<"e164">>)
...
gateway_dialstring(#gateway{route='undefined'
    ...
    Num =
        case Gformat of
            ...
            <<"npan">> -> wnm_util:to_npan(Number);
            <<"1npan">> -> wnm_util:to_1npan(Number);
            <<"national">> -> wnm_util:to_national(Number);
            <<"international">> -> wnm_util:e164_tointernational(Number)
        end,
    Route = list_to_binary(["sip:"
                            ,wh_util:to_binary(GPrefix)
                            ,Num
                            ,wh_util:to_binary(GSuffix)
                            ,"@"
