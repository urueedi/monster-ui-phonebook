...
handle_audio_req(OffnetReq) ->
    ...
    Props = [{<<"Callee-ID-Number">>, Number}
             ,{[<<"Custom-Channel-Vars">>, <<"Callee-ID-Number">>], Number}
            ],
    OffnetReq1 = wh_json:set_values(Props, OffnetReq),
    lager:debug("received outbound audio resource request for ~s: ~p", [Number, OffnetReq1]),
    OffnetReq2 = maybe_lookup_cnam(Number, OffnetReq1),
    handle_audio_req(Number, OffnetReq2).
...
%% @end
%%--------------------------------------------------------------------
-spec maybe_lookup_cnam(number_properties(), wh_json:object()) -> wh_json:object().
maybe_lookup_cnam(Number, JObj) ->
    case wh_number_properties:outbound_cnam_enabled(Number) of
        'true' -> stepswitch_cnam:lookup_outbound(Number, JObj);
        _R -> lager:debug("cnam for ~p is ~p", [Number, _R]), JObj
    end.
