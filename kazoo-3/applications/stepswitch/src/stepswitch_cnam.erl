...
-define(HTTP_CONNECT_TIMEOUT_MS
        ,whapps_config:get_integer(?CONFIG_CAT, <<"http_connect_timeout_ms">>, 700)
       ).
...
lookup(<<_/binary>> = Number) ->
    Num = case ?DISABLE_NORMALIZE of
        'false' -> wnm_util:normalize_number(wnm_util:to_e164(Number));
        'true'  -> Number
    end,
...
lookup(JObj) ->
    ...
    UserId = wh_json:get_ne_value(?CCV(<<"Owner-ID">>), JObj),
    Number = wh_json:get_value(<<"Caller-ID-Number">>, JObj, wh_util:anonymous_caller_id_number()),
    Num = case ?DISABLE_NORMALIZE of
        'false' -> wnm_util:normalize_number(wnm_util:to_e164(Number, AccountId));
        'true'  -> Number
    end,
    lager:debug("stepswitch cnam inbound N:~p A:~p U:~p N:~p", [Number, AccountId, UserId, Num]),
    case wh_cache:fetch_local(?STEPSWITCH_CACHE, cache_key(Num)) of
        {'ok', CNAM} ->
            update_request(JObj, CNAM, 'true');
        {'error', 'not_found'} ->
            CNAM = fetch_cnam(Num, set_phone_number(Num, AccountId, UserId, JObj)),
            update_request(JObj, CNAM, 'false')
    end.

%% -------------------------------------------------------------------

-spec set_phone_number(ne_binary(),ne_binary(),ne_binary(), wh_json:object()) -> wh_json:object().
set_phone_number(Num, AccountId, UserId, JObj) ->
    Props = [{<<"owner_id">>, UserId}, {<<"account_id">>, AccountId}, {<<"phone_number">>, wh_util:uri_encode(Num)}],
...
-spec lookup_outbound(ne_binary(), wh_json:object()) -> wh_json:object().

lookup_outbound(Number, JObj) ->
    AccountId = wh_json:get_ne_value(?CCV(<<"Account-ID">>), JObj),
    UserId = wh_json:get_ne_value(?CCV(<<"Owner-ID">>), JObj),
    Num = wnm_util:normalize_number(wnm_util:to_e164(Number, AccountId)),
    case wh_cache:fetch_local(?STEPSWITCH_CACHE, cache_key(Num)) of
        {'ok', CNAM} ->
            Ret = update_request_outbound(JObj, CNAM, 'true'),
    lager:debug("stepswitch cnam outbound N:~p A:~p U:~p N:~p Ret:~p", [Number, AccountId, UserId, Num, Ret]), Ret;
        {'error', 'not_found'} ->
            CNAM = fetch_cnam(Num, set_phone_number(Num, AccountId, UserId, JObj)),
            Ret = update_request_outbound(JObj, CNAM, 'false'),
    lager:debug("stepswitch cnam outbound N:~p A:~p U:~p N:~p Ret:~p", [Number, AccountId, UserId, Num, Ret]), Ret
    end.

%%%===================================================================

-spec update_request_outbound(wh_json:object(), api_binary(), boolean()) -> wh_json:object().
update_request_outbound(JObj, 'undefined', _) -> lager:debug("stepswitch cnam: undefined JObj:~p", [JObj]), JObj;
update_request_outbound(JObj, CNAM, FromCache) ->
    lager:debug("stepswitch cnam:~p JObj:~p", [CNAM, JObj]),
    Props = [{<<"Outbound-Callee-ID-Name">>, CNAM}
             ,{[<<"Custom-Channel-Vars">>, <<"Outbound-Callee-ID-Name">>], CNAM}
