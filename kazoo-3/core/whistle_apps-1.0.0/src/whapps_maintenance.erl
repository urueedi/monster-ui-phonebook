...
refresh(?WH_LCRRATES_DB) ->
    couch_mgr:db_create(?WH_LCRRATES_DB),
    refresh_lcrrates_db(?WH_LCRRATES_DB);
refresh(?WH_PHONEBOOK_DB) ->
    couch_mgr:db_create(?WH_PHONEBOOK_DB),
    refresh_phonebook_db(?WH_PHONEBOOK_DB);
...
refresh(Database) when is_binary(Database) ->
    case couch_util:db_classification(Database) of
        ...
        'lcrrates' -> refresh_lcrrates_db(Database);
        'phonebook' -> refresh_phonebook_db(Database);
        'system' -> couch_mgr:db_create(Database),
            'ok';
        _Else -> '~p not found',Database
    end.
...
refresh_ratedeck_db(Database) ->
    ...
    lager:info("database ~s refresh: ~p", [Db]),
    couch_mgr:load_fixtures_from_folder(Db, 'hotornot'),
    'ok'.

-spec refresh_lcrrates_db(ne_binary()) -> 'ok'.
refresh_lcrrates_db(Database) ->
    Db = case Database of
             ?WH_LCRRATES_DB -> ?WH_LCRRATES_DB;
             _ -> Database
         end,
    _ = couch_mgr:revise_doc_from_file(Db, 'crossbar', <<"views/lcrrates.json">>),
    lager:info("database ~s refresh: ~p", [Db]),
    couch_mgr:load_fixtures_from_folder(Db, 'lcrroute'),
    'ok'.

-spec refresh_phonebook_db(ne_binary()) -> 'ok'.
refresh_phonebook_db(Database) ->
    Db = case Database of
             ?WH_PHONEBOOK_DB -> ?WH_PHONEBOOK_DB;
             _ -> Database
         end,
    _ = couch_mgr:revise_doc_from_file(Db, 'crossbar', <<"views/phonebook.json">>),
    lager:info("database ~s refresh: ~p", [Db]),
%%    couch_mgr:load_fixtures_from_folder(Db, 'phonebook'),
    'ok'.
...
create_db(Database) when is_binary(Database) ->
    case couch_util:db_classification(Database) of
        'account' -> couch_mgr:db_create(Database),
                     refresh_account_db(Database);
        'modb' -> couch_mgr:db_create(Database),
                     kazoo_modb:refresh_views(Database);
        'ratedeck' -> couch_mgr:db_create(Database),
                    refresh_ratedeck_db(Database);
        'lcrrates' -> Database,couch_mgr:db_create(Database),
                    refresh_lcrrates_db(Database);
        'phonebook' -> Database,couch_mgr:db_create(Database),
                    refresh_phonebook_db(Database);
        'system' -> Database,couch_mgr:db_create(Database),
            'ok';
        _Else -> '~p not found',Database
    end.
